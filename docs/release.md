...
Заглянуть в базу на устройстве (`run-as`) можно только у отлаживаемого приложения, а
переустановка обычной debug-сборкой стирает данные — у неё другая подпись, да и весит она
750 МБ вместе с моделями. Поэтому есть флаг:

```bash
./gradlew assembleRelease -PdebuggableRelease=true
```

Подпись и applicationId те же, что у обычного релиза, поэтому APK встаёт поверх
установленного без потери разметки, а `run-as ai.recommend.spacegallery` работает. Сборка
предупреждает об этом в лог. В публикуемые сборки флаг не попадает — он задаётся только
руками.
